...
{
    /// <summary>Office document (DOCX) operations for agent use: create and update documents from HTML templates. Each document type maps to an HTML template; types without a template get a new one generated by the LLM. Every save creates a new version in the workspace git repo (rollback via GitTool.restore). File paths are Unix-style, relative to the workspace root — never escape it.</summary>
    public class OfficeSupportTool : BaseAgentTool, IFileTool
    {
        ...
        /// <summary>Create an office document (DOCX): the template matching the requested type is filled by the LLM with the provided material, converted to DOCX, and the source HTML is stored inside the DOCX as hidden metadata so the document can be updated later. The new content becomes a new version (rollback via GitTool.restore).</summary>
        ...
        /// <summary>Updates an existing DOCX document on request (e.g. change a value, add a clause, fix a table): the source HTML is read from the embedded document metadata, the requested changes are validated for clarity, the LLM produces the updated HTML, and the file is regenerated and overwritten in place. The new content becomes a new version (rollback via GitTool.restore).</summary>
        ...
        /// <returns>The updated .docx path in workspace form (with the new version id), or an "Error: ..." message (missing input, document without HTML metadata, unclear changes, LLM failure).</returns>
